...
// 2. Checking invariant C>B+A, D>C+B, etc to merge almost/perfectly equal, balanced sub-arrays, Fibonacci link?
// 3. Galloping function reduces no.of comparisons (Assume leftA = {1, 2, 3, ..., 10000}, rightA = {20000, 20001, ...., 30000})
// After sorting leftA & rightA sub-arrays before merging, checks if rightA[0] > leftA[0]-leftA[6] (Initial galloping threshold = 7)
// If so, successful gallop, copy leftA into A. If unsuccessful: ?
// (Each successful gallop: galloping threshold--? Unsuccessful: galloping threshold++? (only for smaller arrays?)
// or n+2^i? (check against [8],[10],[14],[22],[38],...)
public class TimSort {
    private final int runSize = 64; // can be [32-65], how to choose, what's the diff?

    void timSort(int[] A, int n) {
        // Sort adjacent runs (some part will overlap, ensuring stability) eg. 1&2, 2&3
    ...
    // if n>=64, search for runs of >= minrunSize, strictly increasing/decreasing(simply reverse)
    int minrunSize(int z) {
        assert z >= 0 : "N cannot be negative!"; //enable assertions in Java by -ea as arguments, to see error & error msgs
        //if (!(N >= 0)) throw new AssertionError("N cannot be negative!");
...
/** https://hg.openjdk.java.net/jdk8/jdk8/jdk/file/687fd7c7986d/src/share/classes/java/util/TimSort.java
