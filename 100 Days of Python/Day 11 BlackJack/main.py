...
def play():
    ...
    def yourMoves():
        ...
        while input("Type 'y' to draw another card, type 'n' to pass: ")=="y":
            ...
            # Need to figure out how to better implement this... since I used a dictionary to store
                if "A" in yourCards[1]:
                    # Edge Case: when you initially draw 2 As (>1 A) or dealer draws multiple As too
                    for _ in range (yourCards[1]["A"]):
    ...
    def dealerMoves():
        ...
        if dealerCards[0]['score']==21:
            ...
            if "A" in dealerCards[1]:
                for _ in range (dealerCards[1]["A"]):
        ...
        else:evaluateScores(dealerCards[0]['score'],yourCards[0]['score'])

    def prntCurrent(yourCards,dealerCards):
        AtotempA(yourCards,dealerCards)
        print()
    ...
    def prntFinal(yourCards,dealerCards):
        tempAtoA(yourCards,dealerCards)
        print()
    ...
    def AtotempA(yourCards,dealerCards):
        if "A" in yourCards[1]:
            yourCards[1]["tempA"]=yourCards[1]["A"]
            del yourCards[1]["A"]
        if "A" in dealerCards[1]:
            dealerCards[1]["tempA"]=dealerCards[1]["A"]
            del dealerCards[1]["A"]

    def tempAtoA(yourCards,dealerCards):
    ...
    def winMessage(betAmountInBtc,dealerBTC,yourBTC):
        ...
        print(f"You've won {betAmountInBtc} BTC(${format((amountInFiat(betAmountInBtc)),'.2f')})! :)\nYou've {BTC['yourBTC']} BTC(${format((amountInFiat(BTC['yourBTC'])),'.2f')}) left.\n")
        endRound()

    def lossMessage(betAmountInBtc,dealerBTC,yourBTC):
        BTC['dealerBTC']+=betAmountInBtc
        BTC['yourBTC']-=betAmountInBtc
        print(f"You've lost {betAmountInBtc} BTC(${format((amountInFiat(betAmountInBtc)),'.2f')})! :(\nYou've {BTC['yourBTC']} BTC(${format((amountInFiat(BTC['yourBTC'])),'.2f')}) left.\n")
        endRound()
    ...
    while not gameEnd:
        if input("Do you want to play a game of BlackJack? Type 'y' or 'n': ").lower()=="y":
            ...
            assert 0<betAmountInBtc<=BTC['yourBTC'], "You don't have enough!"
...
def play():
    ...
    def youDraw(yourCards):
        ...
        yourCards[0]['score']+=getValue(yourDraw)
        print(f"You drew a {yourDraw}")
        if yourDraw=="A" and "A" in yourCards[1]:  
            if yourCards[1]["A"]==2 and yourCards[2]['count']==2:
                # Edge Case for you initally drawing A,A
                yourCards[1]["tempA"]=2
                yourCards[0]['score']-=10
            elif yourCards[1]["A"]==1 and yourCards[0]['score']==21:
                print("My my! BlackJack!", winMessage(betAmountInBtc,BTC['dealerBTC'],BTC['yourBTC']))
            else:
                yourCards[1]["tempA"]=yourCards[1]["A"]
                if yourCards[1]["tempA"]==1 and yourCards[2]['count']==2:yourCards[0]['score']+=10
                elif yourCards[1]["tempA"]==1 and yourCards[2]['count']>=3: 
                    for _ in range (yourCards[1]["A"]+1):yourCards[0]['score']-=10
                del yourCards[1]["A"]
    ...
    def dealerDraws(dealerCards):
        ...
        dealerCards[0]['score']+=getValue(dealerDraw)
        print(f"Dealer drew a {dealerDraw}")
        if dealerDraw=="A" and "A" in dealerCards[1]:     
            if dealerCards[1]["A"]==2 and dealerCards[2]['count']==2:
                yourCards[1]["tempA"]=2
                dealerCards[0]['score']-=10
            elif dealerCards[1]["A"]==1 and dealerCards[0]['score']==21:
                # Dealer Blackjack
                pass
            else:
                dealerCards[1]["tempA"]=dealerCards[1]["A"]
                if dealerCards[1]["tempA"]==1 and dealerCards[2]['count']==2:yourCards[0]['score']+=10
                elif dealerCards[1]["tempA"]==1 and dealerCards[2]['count']>=3: 
                    for _ in range (dealerCards[1]["A"]+1):dealerCards[0]['score']-=10
            # Bug to fix: After having an A, 3rd and onwards dealer draw, dealer's score need to -10
                del dealerCards[1]["A"]
    ...
    def tempAtoA(yourCards,dealerCards):
        if "tempA" in yourCards[1]:
            if yourCards[1]["tempA"]==1 and yourCards[2]['count']==2:
                for _ in range (yourCards[1]["tempA"]):yourCards[0]['score']+=10
            yourCards[1]["A"]=yourCards[1]["tempA"]
            del yourCards[1]["tempA"]
        if "tempA" in dealerCards[1]:
            dealerCards[1]["A"]=dealerCards[1]["tempA"]
